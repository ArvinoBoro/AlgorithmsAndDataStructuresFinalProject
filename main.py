...
def main():
    ...
    with open('adjacencies.csv', newline='') as csv_file:
        ...
        for row in csv_reader: # O(E)
            if not map.vertex_exists(row[0]): # O(V)
                map.add_vertex(row[0]) # O(E * V)

            if not map.vertex_exists(row[1]):
                map.add_vertex(row[1]) # O(E * V)

            map.add_edge(row[0], row[1], int(row[2])) # O(E^2 + EV)
